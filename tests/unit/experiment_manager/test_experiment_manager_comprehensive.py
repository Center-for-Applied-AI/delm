...
class TestInMemoryExperimentManagerComprehensive:
    ...
    def setup_method(self):
        """Set up test manager."""
        self.manager = InMemoryExperimentManager()

    def test_initialization_with_invalid_kwargs(self):
        """Test initialization with unsupported kwargs."""
        # InMemoryExperimentManager now takes no arguments
        # Any arguments passed should result in a TypeError
        with pytest.raises(TypeError):
            InMemoryExperimentManager(experiment_name="test")

        with pytest.raises(TypeError):
            InMemoryExperimentManager(overwrite_experiment=True)

        with pytest.raises(TypeError):
            InMemoryExperimentManager(auto_checkpoint_and_resume_experiment=True)
    ...
    def test_load_preprocessed_data_with_file_path(self):
        ...
        with pytest.raises(
            NotImplementedError,
            match="Loading preprocessed data from a file path is not supported",
        ):
    ...
    def test_load_preprocessed_data_without_data(self):
        ...
        with pytest.raises(
            ValueError, match="No preprocessed data available in memory"
        ):
    ...
    def test_load_batch_checkpoint_with_invalid_path(self):
        ...
        with pytest.raises(
            ValueError, match="No batch checkpoint with id 999 in memory"
        ):
    ...
    def test_consolidate_batches_empty(self):
        ...
        with pytest.raises(
            ValueError, match="No batch checkpoints in memory to consolidate"
        ):
...
class TestDiskExperimentManagerComprehensive:
    ...
    def setup_method(self):
        ...
        experiment_path = self.tmp_path / "experiments" / "test_experiment"
    ...
    def test_initialization(self):
        ...
        # experiment_dir is the full path to the experiment
        assert self.manager.experiment_dir.name == "test_experiment"
    ...
    def test_is_experiment_completed_true(self):
        ...
        result_file = self.manager.data_dir / CONSOLIDATED_RESULT_FILE_NAME
    ...
    def test_get_results_file_not_found(self):
        ...
        with pytest.raises(
            FileNotFoundError, match="Consolidated result file does not exist"
        ):
            self.manager.get_results()
    ...
    def test_initialize_experiment_new_directory(self):
        ...
        config_file = self.manager.config_dir / "config.yaml"
    ...
    def test_initialize_experiment_existing_completed(self):
        ...
        with pytest.raises(
            ...
            match="Experiment exists and is already completed",
        ):
            self.manager.initialize_experiment(config)
    ...
    def test_save_preprocessed_data(self):
        ...
        assert result_path.name.endswith(".feather")
    ...
    def test_load_preprocessed_data_wrong_extension(self):
        ...
        with pytest.raises(
            ValueError, match="Preprocessed data file must be a feather file"
        ):
    ...
    def test_save_batch_checkpoint(self):
        ...
        assert result_path.exists()
        expected_name = (
            f"{BATCH_FILE_PREFIX}{1:0{BATCH_FILE_DIGITS}d}{BATCH_FILE_SUFFIX}"
        )
    ...
    def test_load_batch_checkpoint_file_not_found(self):
        ...
        with pytest.raises(
            FileNotFoundError, match="Batch checkpoint file does not exist"
        ):
    ...
    def test_load_batch_checkpoint_wrong_extension(self):
        ...
        with pytest.raises(
            ValueError, match="Batch checkpoint file must be a feather file"
        ):
    ...
    def test_consolidate_batches_no_files(self):
        ...
        with pytest.raises(
            FileNotFoundError, match="No batch files found for consolidation"
        ):
            self.manager.consolidate_batches()
    ...
    def test_save_extracted_data(self):
        ...
        assert result_path.exists()
        assert result_path.name.startswith(CONSOLIDATED_RESULT_FILE_NAME)
        assert result_path.name.endswith(".feather")
...
class TestExperimentManagerEdgeCases:
    ...
    def test_inmemory_batch_path_parsing_edge_cases(self):
        ...
        manager = InMemoryExperimentManager()
    ...
    def test_disk_batch_id_parsing_edge_cases(self):
        ...
        try:
            experiment_path = tmp_path / "experiments" / "test"
            manager = DiskExperimentManager(experiment_path)
    ...
    def test_large_dataframes(self):
        ...
        large_df = pd.DataFrame(
            {"col1": range(10000), "col2": [f"string_{i}" for i in range(10000)]}
        )
