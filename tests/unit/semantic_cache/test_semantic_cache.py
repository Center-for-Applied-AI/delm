...
class TestUtilities:
    ...
    def test_make_semantic_key(self):
        ...
        material = {"prompt": "test prompt", "model": "gpt-4", "temperature": 0.1}
...
class TestLMDBCache:
    ...
    def test_get_hit(self, tmp_path):
        """Test cache get with hit."""
        with patch("delm.utils.semantic_cache.lmdb") as mock_lmdb, patch(
            "delm.utils.semantic_cache.zstd"
        ) as mock_zstd:
            mock_env = Mock()
            mock_lmdb.open.return_value = mock_env
...
class TestSemanticCacheFactory:
    ...
    def test_from_config_dict(self):
        ...
        with patch("delm.utils.semantic_cache.Path") as mock_path:
    ...
    def test_from_config_dataclass(self):
        ...
        with patch("delm.utils.semantic_cache.Path") as mock_path, patch(
            "sqlite3.connect"
        ) as mock_connect:
            mock_path.return_value = Path("/tmp/test_cache")
            mock_db = Mock()
    ...
    def test_from_config_lmdb(self):
        ...
        with patch("delm.utils.semantic_cache.Path") as mock_path, patch(
            "delm.utils.semantic_cache.lmdb"
        ) as mock_lmdb:
