...
def make_cache_key(
    *,
    ...
    Build a deterministic cache key that depends **only** on:
      ...
      • temperature
    """
    log.debug("Creating cache key: model=%s, temperature=%s, prompt_length=%d, system_length=%d", 
             model_name, temperature, len(prompt_text), len(system_prompt))
    material = {
        "prompt"   : prompt_text,
...
class FilesystemJSONCache(SemanticCache):
    ...
    def get(self, key: str) -> Optional[bytes]:
        ...
        if p.exists():
            ...
            log.debug("FilesystemJSONCache hit: key=%s, data_size=%d bytes", key[:16] + "...", len(data))
            return data
        self._miss += 1
    ...
    def set(self, key: str, value: bytes, meta: Mapping[str, Any] | None = None) -> None:
        p = self._path(key)
        log.debug("FilesystemJSONCache set: key=%s, path=%s, value_size=%d bytes", key[:16] + "...", p, len(value))
        p.parent.mkdir(parents=True, exist_ok=True)
        compressed = self._compress(value)
        p.write_bytes(compressed)
        log.debug("FilesystemJSONCache stored: key=%s, compressed_size=%d bytes", key[:16] + "...", len(compressed))
        # meta goes in a sidecar .meta for transparency
        if meta:
            meta_path = p.with_suffix(".meta.json")
            meta_path.write_text(_canonical_json(meta))
            log.debug("FilesystemJSONCache stored metadata: key=%s, meta_path=%s", key[:16] + "...", meta_path)
    ...
    def stats(self):
        ...
        stats = {"backend": "filesystem", "entries": total, "bytes": size, "hit": self._hits, "miss": self._miss}
    ...
    def prune(self, *, max_size_bytes: int):
        ...
        log.debug("FilesystemJSONCache current size: %d bytes, %d files", size, len(files))
        ...
        log.debug("FilesystemJSONCache pruning completed: deleted %d files, final size %d bytes", deleted_count, size)
...
class SQLiteWALCache(SemanticCache):
    ...
    def __init__(self, path: Path, synchronous: str = "NORMAL"):
        log.debug("Initializing SQLiteWALCache with path: %s, synchronous: %s", path, synchronous)
    ...
    def get(self, key: str) -> Optional[bytes]:
        ...
        with self.env.begin(buffers=True) as txn:
            ...
            if self._d:
                ...
                log.debug("LMDBCache hit: key=%s, compressed_size=%d, decompressed_size=%d", 
                         key[:16] + "...", len(val), len(decompressed))
                return decompressed
            else:
                log.debug("LMDBCache hit: key=%s, data_size=%d", key[:16] + "...", len(val))
                return bytes(val)

    def set(self, key: str, value: bytes, meta: Mapping[str, Any] | None = None):
        log.debug("LMDBCache set: key=%s, value_size=%d bytes", key[:16] + "...", len(value))
        payload = self._c.compress(value) if self._c else value
        log.debug("LMDBCache compressed: key=%s, original_size=%d, compressed_size=%d", 
                 key[:16] + "...", len(value), len(payload))
