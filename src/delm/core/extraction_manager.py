...
class ExtractionManager:
    ...
    def __init__(
        ...
        log.debug(f"Creating ConcurrentProcessor with max_workers: {model_config.max_workers}")
    ...
    def process_with_batching(
        ...
        log.debug("Starting batch processing with %d chunks, batch_size=%d", len(text_chunks), batch_size)
        ...
        if auto_checkpoint:
            ...
            log.debug("Found %d unverified batch IDs: %s", len(unverified_batch_ids), sorted(unverified_batch_ids))
        else:
        ...
        for batch_id in sorted(unverified_batch_ids):
            try:
                ...
                if batch_df is not None:
                    ...
                    log.debug("Batch %d verified successfully with %d chunks", batch_id, chunk_count)
                else:
        ...
        log.debug("Verified %d batches (%d chunks), corrupted %d batches", 
                len(verified_batch_ids), already_processed_chunks, len(corrupted_batch_ids))

        # 3. Delete corrupted batch files so they can be replaced
        if corrupted_batch_ids:
            log.debug("Deleting %d corrupted batch checkpoints", len(corrupted_batch_ids))
            for batch_id in corrupted_batch_ids:
                try:
                    ...
                    if deleted:
                        log.debug("Successfully deleted corrupted batch checkpoint %d", batch_id)
                    else:
                        log.warning("Failed to delete corrupted batch checkpoint %d (delete returned False)", batch_id)
                except Exception as e:
        ...
        log.debug("Batch calculation: total_chunks=%d, batch_size=%d, total_batches=%d", total_chunks, batch_size, total_batches)
        log.debug("Batches to process: %s", batches_to_process)
        log.debug("Processing %d batches out of %d total (already processed %d chunks)", len(batches_to_process), total_batches, already_processed_chunks)
        ...
        log.debug("Setting up progress bar with total=%d, initial=%d", total_chunks, already_processed_chunks)
        with tqdm(
            total=total_chunks,
            ...
            for batch_id in batches_to_process:
                ...
                log.debug("Processing batch %d with %d chunks (range: %d-%d)", batch_id, len(batch_chunks), start, end)
                
                # Check if we are over budget
                if self.track_cost and self.cost_tracker.is_over_budget():
                    log.warning("Over budget, stopping extraction at batch %d", batch_id)
                    break
                ...
                if auto_checkpoint:
                    ...
                    log.debug("Successfully saved batch checkpoint %d and state", batch_id)
                else:
                    log.debug("Adding batch %d DataFrame to memory collection", batch_id)
        ...
        if auto_checkpoint:
            ...
            log.debug("Auto-checkpoint disabled, concatenating batch DataFrames from memory")
            consolidated_df = pd.concat(batch_dfs, ignore_index=True)
            log.debug("Memory concatenation completed")

        log.debug("Batch processing completed. Final results: %d records", len(consolidated_df))
    ...
    def _instructor_extract_with_retry(self, text_chunk: str) -> BaseModel:
        ...
        log.debug("Extraction setup: provider=%s, prompt_length=%d, system_prompt_length=%d", 
        ...
        def _instructor_extract():
            ...
            try:
                log.debug("Making LLM API call: model=%s, temperature=%s", self.model_config.name, self.temperature)
